...
#define PROC_NAME "<DaviHenrique30>_PROC_FS"

static void *my_seq_start(struct seq_file *s, loff_t *pos)
{
	static unsigned long counter = 0;

	/* inicia uma nov sequencia */
	if (*pos == 0) {
...
{
	/* nada ser feito, usamos um valor estatico no start()  */
}
...
static int __init proc_init(void)
{
	...
	if (entry == NULL) {
		...
		pr_debug("Erro: Não foi possivel iniciliar /proc/%s\n", PROC_NAME);
		return -ENOMEM;
	}
